...
public static class App
{
    ...
    public static async Task RunAsync(Args args)
    {
        ...
        if (IsLocal)
        {
            Logger.Log($"App Version: {ServerVersion}");
    ...
    public static BuildPipeline CreateBuildPipeline(Workspace workspace, Args args, Project project)
    {
        ...
        Pipelines.Add(project.Guid!, pipeline);
    ...
    private static async Task DeployGoogle(BuildPipeline pipeline, string buildVersionTitle)
    {
        ...
        var packageName = pipeline.Workspace.ProjectSettings.GetValue<string>(
            "applicationIdentifier.Android"
        );
        var changeLogArr = pipeline.ChangeLog;
        var changeLog = string.Join("\n", changeLogArr);
        var buildSettingsAsset = pipeline.Workspace.GetBuildTarget(
            BuildTargetFlag.Android.ToString()
        );
        ...
        try
        {
            if (Config?.Clanforge != null)
            {
                pipeline.Args.TryGetArg(
                    "-clanforge",
                    out var profile,
                    Config.Clanforge.DefaultProfile
                );
