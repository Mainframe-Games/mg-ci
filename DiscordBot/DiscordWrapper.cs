...
public class DiscordWrapper
{
    ...
    public DiscordWrapper(DiscordConfig config)
    {
        ...
        // TODO: implement new socket server
        // listen server
    ...
    private async Task SlashCommandHandler(SocketSlashCommand command)
    {
        ...
        var cmd = Commands.FirstOrDefault(x => command.CommandName == x.CommandName);
        var commandFull =
            $"`/{command.CommandName} {string.Join(" ", command.Data?.Options?.Select(x => $"{x.Name} {x.Value}") ?? Array.Empty<string>())}`";

        // execute or fail
        if (cmd == null)
        {
            await command.RespondError(
                "Not Recognised",
                $"Command not recognised: {command.CommandName}"
            );
            return;
        }
        ...
        var restInteractionMessage = await command.RespondSuccessDelayed(
            command.User,
            buildCommand.Embed,
            commandFull
        );
    ...
    private async Task CreateMessageUpdater(
        IDiscordInteraction command,
        RestInteractionMessage? restInteractionMessage,
    ...
    )
    {
        ...
        var firstReport = _prematureReports.TryGetValue(command.Id, out var report)
            ? report
            : new PipelineReport();
    ...
    private bool TryGetCommand<T>(out T command)
        where T : Command
    {
        foreach (var cmd in Commands)
        {
