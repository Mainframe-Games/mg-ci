...
public class UnityBuildPipeline
{
    ...
    public delegate Task<bool> DeployDelegate(UnityBuildPipeline pipeline);
    ...
    /// </summary>
    private readonly List<BuildResult> _buildResults = [];
    ...
    public UnityBuildPipeline(
        string projectId,
        Workspace workspace,
        Args args,
        IOffloadable? offloadable
    )
    {
        ProjectId = projectId;
        Workspace = workspace;
    ...
    private async Task Build()
    {
        ...
        {
            if (
                _offloadable?.IsOffload(
                    build,
                    BuildVersions,
                    _currentChangeSetId,
                    Args.IsFlag("-cleanbuild")
                )
                is true
            )
            {
    ...
    private void PostBuild()
    {
        ...
        var title =
            $"{Workspace.Meta?.ProjectName ?? Workspace.Name} | {BuildVersions?.FullVersion}";
    ...
    public void SetOffloadBuildStatus(
        string buildGuid,
        string buildName,
        BuildTaskStatus buildTaskStatus,
    ...
    )
    {
        ...
        Logger.Log(
            $"Offload Status: {buildName}, status: {buildTaskStatus}, result: {buildResult}"
        );
        ...
        switch (buildTaskStatus)
        {
            ...
            case BuildTaskStatus.Failed:
                ...
                Report.Complete(
                    ...
                    buildResult?.Errors?.ToString() ?? "Reason unknown"
                );
                Cancel();
                break;
