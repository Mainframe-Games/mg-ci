...
internal class Plastic(string projectPath)
{
    public void Update() { }
    ...
    public void Commit(string commitMessage, IEnumerable<string> filesToCommit)
    {
        ...
        Cmd.Run("cm", $"ci {filesStr} -c=\"{commitMessage}\"", workingDirectory: projectPath);
    }

    public void SwitchBranch(string branchPath)
    {
        var res = Cmd.Run("cm", $"switch {branchPath} --workspace=\"{projectPath}\"");
        if (res.exitCode != 0)
    ...
    public string GetLatestCommitHash()
    {
        var res = Cmd.Run(
            "cm",
            "log --format=\"{changeset} {date} {comment}\" --limit=1",
    ...
    {
        var raw = Cmd.Run("cm", $"log --from=cs:{0} cs:{0} --csformat=\"{{comment}}\"").output;
...
// internal class PlasticWorkspace(string name, string directory, string projectId)
